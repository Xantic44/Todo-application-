...
// handleSubmit now for submit capturing !
handleSubmit= (e) => {
  e.preventDefault();

  //create new item, new value place in new !
  const newItem = {
    id: this.state.id,
...
// delete tasks method by handleDelete.
handleDelete= (id) => {
...
//handle edit method. for editing task if typos.
handleEdit = id => {
const filteredItems = this.state.items.filter(item => item.id !== id)
